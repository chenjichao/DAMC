function norm_dists_star = distancefusion_weighted(X, fusion1, fusion2, A)
...
for iViw = 1:nViw
    ...
    W(iViw,:,:) = 0.5/sqrt(sum(sum((squeeze(norm_dists_all(iViw,:,:)).*A))));
    norm_dists_all(iViw,:,:) = W(iViw).*norm_dists_all(iViw,:,:);
end

% distance fusion between views
switch lower(fusion2)
    case {'gm','geometric mean'}
%         norm_dists_star = squeeze(geomean(norm_dists_all, 1));
        norm_dists_star = squeeze(prod(norm_dists_all,1));
    case {'am','arithmetic mean'}
%         norm_dists_star = squeeze(mean(norm_dists_all, 1));
        norm_dists_star = squeeze(sum(norm_dists_all,1));
