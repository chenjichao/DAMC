function result = ClusteringPerformance(Y, predY)
% Y and predY should have size of [nSmp, 1]
if size(Y,2) ~= 1
    Y = Y';
end
if size(predY,2) ~= 1
...
nSmp = length(Y);
% Y and predY should have values in 1:nCls
predLidx = unique(predY);
pred_nCls = length(predLidx);
if pred_nCls ~= max(predY)
    predY0 = zeros(nSmp,1);
    for i = 1:pred_nCls
        predY0(predY == predLidx(i)) = i;
    end
    predY = predY0;
end
uY = unique(Y);
...
[f p r] = compute_fpr(Y, predY); % F-score, precision, recall

ARI = RandIndex(Y, predY);

result = [ACC, NMI, Purity, f, p, r, ARI];
...
%%
function [f, p, r] = compute_fpr(T, H) % F-score, precision, recall
